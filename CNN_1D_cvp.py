...
class MyHyperModel(kt.HyperModel):
    ...
    def build(self, hp):
        ...
        hp_dropouts = hp.Float('dropouts', min_value=0.10, max_value=0.5, step=0.1)
...
def get_tuner(size, name):
    ...
    directory="logs/cvp", # directory to save logs 
    project_name=name)
...
def model_eval(model, x_test, y_test, file):
    ...
    save_path = "results/cvp/confusion/"+file[:-4]+".png"
    ...
    save_path = "results/cvp/ROC/"+file[:-4]+".png"
    fig.write_image(save_path)

def main():
    #get list of files in datasets/ path
    files = [f for f in listdir("datasets/cvp/") if (isfile(join("datasets/cvp/", f)))]

    for file in files:
        #Load Dataset
        x_train, y_train, x_val, y_val, x_test, y_test = load_dataset("datasets/cvp/"+file)
        ...
        checkpoint_path = "best_weights/cvp/" +file_short + "_weights.h5"
        ...
        #SAVE MODEL
        save_path = "models/cvp/"+file_short+".h5"
